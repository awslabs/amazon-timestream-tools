...
public class UnloadUtil {
    ...
    public UnloadUtil(final AmazonTimestreamQuery queryClient,
    ...
    public void runAllQueries() throws IOException {
        for (int i = 0; i < queries.size(); i++) {
            String queryString = queries.get(i).getUnloadQuery();
            System.out.println("Running UNLOAD query" + (i + 1) + ": " + queryString);
            List<QueryResult> queryResults = runQuery(queryClient, queryString);
            displayResults(queryResults, queries.get(i));
        }
    }
    ...
    public void runSingleQuery() throws IOException {
        ...
        List<QueryResult> queryResults = runQuery(queryClient, queryString);
        displayResults(queryResults, queries.get(0));
    }

    public UnloadManifest getUnloadManifest(UnloadResponse unloadResponse) throws IOException {
        S3Object s3Object = timestreamDependencyHelper.getObject(unloadResponse.getManifestFile());
    ...
    public UnloadMetadata getUnloadMetadata(UnloadResponse unloadResponse) throws IOException {
        S3Object s3Object = timestreamDependencyHelper.getObject(unloadResponse.getMetadataFile());
    ...
    public void displayResults(List<QueryResult> queryResults, UnloadQuery unloadQuery) throws IOException {
        UnloadResponse unloadResponse = Streams.findLast(queryResults.stream())
                .map(e -> {
                    Preconditions.checkArgument(e.getRows().size() == 1);
                    Map<String, String> outputMap = new HashMap<>();
                    for (int i = 0; i < e.getColumnInfo().size(); i++) {
                        outputMap.put(e.getColumnInfo().get(i).getName(),
                                e.getRows().get(0).getData().get(i).getScalarValue());
    ...
    private void addParquetRecordsFromFile(List<List<String>> mergedResults, UnloadManifest.ResultFile resultFile) throws IOException {
    ...
    private static void addCSVRecordsFromFile(List<List<String>> mergedResults, S3Object s3Object) throws IOException {
        try (CSVParser reader = new CSVParser(new BufferedReader(new InputStreamReader(new GZIPInputStream(
                s3Object.getObjectContent()))), CSVFormat.DEFAULT)) {

...
go run query-sample.go -query "SELECT BIN(time, 30s) AS binned_timestamp, ROUND(AVG(cpu_utilization), 2) AS avg_cpu_utilization,
hostname FROM $1.$2
WHERE measure_name = 'metrics'
    AND hostname = '$3'
    AND time > ago(2h)
...
go run query-operations.go -query "SELECT * FROM $1.$2"

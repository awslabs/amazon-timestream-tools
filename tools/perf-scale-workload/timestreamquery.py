...
def createQueryClient(region, retries = 5, readTimeout = 335, profile = None, endpoint=None):
    ...
    print("Connecting to Timestream service in ", region)
...
## as a dataframe.
def executeQueryAndReturnAsDataframe(client, query, timing = False):
    return flatModelToDataframe(executeQuery(client, query, timing))

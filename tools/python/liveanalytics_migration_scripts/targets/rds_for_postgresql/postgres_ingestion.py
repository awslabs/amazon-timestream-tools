...
def _execute_copy_command(cur, table_name, csv_file, column_names, logger):
    ...
    logger.info(
        f"Rows ingested: {rows_ingested}, Time taken: {minutes} minutes and {seconds} seconds"
    )
...
def _handle_retry_backoff(
    retry_count, max_retries, initial_backoff, backoff_multiplier, logger
):
    """
    ...
    if retry_count > 0:
        ...
        logger.info(
            f"Retry attempt {retry_count}/{max_retries} after {backoff_time} seconds..."
        )
...
def _handle_failure(
    ...
    if retry_count > max_retries:
        error_type = (
            "Database connection error" if is_connection_error else "Error loading"
        )
        ...
        if timestream_utility is not None:
            timestream_utility.sns_publish_message(
                message, "Ingestion to postgres failed"
            )
    else:
        attempt_type = "Connection attempt" if is_connection_error else "Attempt"
        logger.error(
            f"{attempt_type} {retry_count}/{max_retries} failed: {str(exception)}"
        )
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    with open(csv_filepath, "r") as f:
                        ...
                        f.seek(0)
                        rows_ingested = _execute_copy_command(
                            cur, table_name, f, column_names, logger
                        )
...
def decompress_gzip_files(gz_files):
    ...
    for gz_file_path in gz_files:
        extracted_file_path = (
            gz_file_path[:-3] if gz_file_path.endswith(".gz") else gz_file_path
        )
...
def list_files(glob_pattern, file_extension, logger):
    ...
    try:
        ...
        files_with_extension = [
            file for file in all_files if file.lower().endswith(file_extension)
        ]

        logger.info(
            f"Found {len(files_with_extension)} {file_extension} files matching pattern {glob_pattern}"
        )
...
def thread_handler(
    ...
    while True:
        ...
        success, rows = copy_to_postgres(
            conn_pool, table_name, csv_file, processed_dir, logger, timestream_utility
        )
        if success:
    ...
    logger.info(
        f"Thread completed: Processed {files_processed} files with {total_rows_ingested} total rows"
    )
...
def main(input_args):
    ...
    parser.add_argument(
        "-t", "--database", help="postgres database name", required=True
    )
    parser.add_argument(
        "-s",
        "--schema",
        help="Postgres schema where table resides",
        default="public",
        required=False,
    )
    parser.add_argument(
        "-u",
        "--user",
        help="User to login to postgres",
        default="postgres",
        required=False,
    )
    parser.add_argument(
        "-f",
        "--input-files",
        help='Directory or glob pattern for CSV files(Compressed on uncompressed), for example "./data/*.gz"',
        required=True,
    )
    parser.add_argument("-e", "--host", help="Postgres Writer Endpoint", required=True)
    parser.add_argument(
        "-p", "--port", help="Postgres Port", default="5432", required=False
    )
    parser.add_argument(
        "-sm",
        "--secret-arn",
        help="Secrets Manager secret arn if secret stored in Secrets Manager. Omit if using self managed credentials and enter the database password through standard input.",
        required=False,
    )
    parser.add_argument(
        "-pt",
        "--parallel-threads",
        help="Number of threads that will ingest CSV files in parallel",
        default=10,
        required=False,
    )
    parser.add_argument(
        "-pd",
        "--processed-dir",
        help="location for moving the processed files",
        default=None,
        required=False,
    )
    parser.add_argument(
        "-ld",
        "--logs_dir",
        help="Directory for postgres ingestion logs (default: postgres-ingestion-logs)",
        default=None,
        required=False,
    )
    parser.add_argument(
    ...
    args = parser.parse_args(input_args)  # pyright: ignore[reportUnknownArgumentType]

    secret = None
    if args.secret_arn is None:
        secret = input(
            "No value provided for secret-arn. Add the secret-arn for the database credentials, "
    ...
    if log_dir is None:
        log_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "postgres-ingestion-logs"
        )
    os.makedirs(log_dir, exist_ok=True)
    custom_logger_file = (
        f"postgres_ingestion_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    )
    logger = create_logger(
        "postgres_ingestion", log_file=f"{log_dir}/{custom_logger_file}"
    )
    ...
    if not validate_sql_identifier(table_name) or not validate_sql_identifier(schema):
        logger.error(
            "Invalid table name or schema. Must contain only alphanumeric characters and underscores."
        )
        sys.exit(1)
    ...
    if secret is None:
        try:
            ...
            if "password" not in secret:
                error_message = "Password not found in secret)"
            ...
            logger.error(error_message)
            timestream_utility.sns_publish_message(
                error_message, "Failed to retrieve database credentials"
            )
            sys.exit(1)
    ...
    try:
        ...
        timestream_utility.sns_publish_message(
            sns_topic_arn, message, "Database connection failed"
        )
        sys.exit(1)
    except Exception as e:
        ...
        timestream_utility.sns_publish_message(
            sns_topic_arn, message, "Database connection failed with unexpected error"
        )
        sys.exit(1)
    ...
    if not check_table_exists(conn_pool, table_name, schema):
        logger.error(
            f"Table {table_name} does not exist or doesn't exist under provided schema. Please create it first."
        )
        sys.exit(1)
    ...
    try:
        if args.processed_dir is not None:
            ...
            processed_dir = os.path.join(
                input_dir, f"processed_{table_name}_{database_name}"
            )
            logger.info(
                f"Processed files will be moved to: {processed_dir} after processing"
            )

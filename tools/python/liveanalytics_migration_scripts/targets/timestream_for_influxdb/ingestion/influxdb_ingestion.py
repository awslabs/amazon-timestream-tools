...
def check_required_env_vars(skip_bucket_check):
    required_vars = ["INFLUXDB_V2_URL", "INFLUXDB_V2_TOKEN"]
    ...
    try:
        ...
        if bucket_exists:
            ingestion_logger.info(
                f"Bucket '{bucket_name}' exists. Proceeding with ingestion."
            )
            return True
        else:
            ingestion_logger.error(
                f"Bucket '{bucket_name}' does not exist. Create the bucket before ingestion."
            )
            return False
...
def check_org_exists():
    ...
    try:
        ...
        ingestion_logger.error(
            "Error checking if organization '%s' exists: %s", client.org, exc
        )
        return False
    finally:
        if client:
...
def update_tracking_file(file_path, file_name, success=True):
    ...
    try:
        ...
        log_message = (
            f"{'Successfully ingested' if success else 'Failed to ingest'}: {file_name}"
        )
...
def find_latest_ingested_files(tracking_directory):
    ...
    if os.path.exists(prev_success_log):
        try:
            ...
            ingestion_logger.info(
                f"Found {len(ingested_files)} previously ingested files in {prev_success_log}"
            )
        except Exception as e:
...
def decompress_gzip_file(gz_file_path):
    ...
    extracted_file_path = (
        gz_file_path[:-3] if gz_file_path.endswith(".gz") else f"{gz_file_path}.line"
    )
...
def ingest_batch(
    write_api, batch, batch_id, process_name, bucket_name, max_retries, precision
):
    """
    ...
    while ingestion_attempt <= max_retries:
        try:
            ...
            write_api.write(
                bucket=bucket_name, record=batch_content, write_precision=precision
            )
...
def ingest_gzip_file(
    ...
    total_lines = ingest_line_protocol_file(
        extracted_file_path,
        lines_per_batch,
        io_multiplier,
        bucket_name,
    ...
    if extracted_file_path and os.path.exists(extracted_file_path):
        try:
            ...
            ingestion_logger.error(
                "Error deleting extracted file %s: %s", extracted_file_path, exc
            )
...
def poll_for_result(
    result, failure_flag, continue_on_error, failed_log, file_name, poll_interval=5.0
):
    """
        ...
        try:
            ...
            if failure_flag.value == 1:
                ...
                if continue_on_error:
                    ingestion_logger.warning(
                        f"Error detected in file {file_name}, but continuing with next file"
                    )
                    return 0
                else:
                    ingestion_logger.error(
                        "Stopping ingestion due to error in file %s.", file_name
                    )
                    ingestion_logger.error(
                        "Fix the issue and run with --resume-from with the path to the previous tracking folder."
                    )
                    sys.exit(1)
            continue
            ...
            if continue_on_error:
                ingestion_logger.warning(
                    f"Continuing with next file after error {e} in {file_name}"
                )
                return 0
            else:
                ingestion_logger.error(
                    f"Stopping ingestion due to error {e} in file {file_name}."
                )
                ingestion_logger.error(
                    "Fix the issue and run with --resume-from with the path to the previous tracking folder."
                )
...
def main(input_args):
    ...
    parser = argparse.ArgumentParser(
        description="Process gzip files in a directory using multiple processes"
    )
    ...
    parser.add_argument(
        "--logs-dir",
        type=str,
        default="influxdb-ingestion-logs",
    ...
    if not args.skip_bucket_check and (
        not check_bucket_exists(args.bucket) or not check_org_exists()
    ):
        sys.exit(1)
    ...
    if not gz_files:
        ingestion_logger.warning(
            f"No .gz files found to ingest in {args.data_directory}"
        )
        return 0

    ingestion_logger.info(
        f"Found {len(gz_files)}.gz files in directory {args.data_directory}"
    )
    ingestion_logger.info(
        f"Using {args.workers} workers to handle extraction and ingestion"
    )

    start_time = time.time()
    total_lines_ingested = 0
    ingestion_logger.info(
        f"Continue-on-error mode: {'enabled' if args.continue_on_error else 'disabled'}"
    )
    ...
    with Pool(processes=args.workers) as pool:
        ...
        for file_path, result in async_results:
            ...
            lines = poll_for_result(
                result,
                failure_flags[file_name],
            ...
            if lines > 0:
                ...
                ingestion_logger.warning(
                    f"Continuing with next file after error in {file_name}"
                )

    # Log summary statistics
    total_time = time.time() - start_time
    ingestion_logger.info(
        "Unload ingestion complete with a processing time: %.2f seconds", total_time
    )
    ingestion_logger.info("Total number of lines ingested: %d", total_lines_ingested)
    if total_time > 0:
        ingestion_logger.info(
            "Overall ingestion rate: %.2f lines/second",
            total_lines_ingested / total_time,
        )

    successful_count = (
        sum(1 for _ in open(success_log, encoding="utf-8"))
        if os.path.exists(success_log)
        else 0
    )
    failed_count = (
        sum(1 for _ in open(failed_log, encoding="utf-8"))
    ...
    if failed_count > 0:
        ...
        ingestion_logger.error(
            "To retry failed files, run the script with the --resume-from with the path to the previous tracking folder."
        )
